...
function SmartPark() {
  ...
  const current = modules.find((item) => item.id === active) ?? { id: "overview" as const, label: "园区大屏总览", short: "总览", icon: Map };
...
function Iot({ tick }: { tick: number }) { const devices: Array<[LucideIcon, string, string, string, string]> = [[Wind,"暖通空调","1,286","98.2%","cyan"],[Building2,"智慧电梯","108","99.1%","blue"],[Lightbulb,"智慧照明","8,620","97.6%","violet"],[Waves,"给排水泵","326","96.8%","green"]]; return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{devices.map(([icon,label,value,trend,tone]) => <Kpi key={label} icon={icon} label={label} value={value} unit="台" trend={`在线率 ${trend}`} tone={tone} />)}</div><SectionGrid><GlassCard title="设备健康总指数" className="xl:col-span-4"><Chart kind="gauge" tick={tick} className="h-[300px]" /></GlassCard><GlassCard title="设备故障频次分析" className="xl:col-span-8"><Chart kind="bar" className="h-[300px]" /></GlassCard><GlassCard title="关键设备实时遥测" className="xl:col-span-5"><Telemetry /></GlassCard><GlassCard title="维保工单调度" extra={<Button size="sm">新建工单</Button>} className="xl:col-span-7"><DataTable type="iot" /></GlassCard></SectionGrid></div>; }
function Telemetry() { const readings: Array<[string, string, LucideIcon]> = [["冷机出水温度","7.2°C",Thermometer],["电梯振动值","0.18mm/s",Activity],["水泵压力","0.42MPa",Gauge],["照明回路电流","12.8A",Zap]]; return <div className="grid grid-cols-2 gap-3 pt-2">{readings.map(([name,value,Icon]) => <div className="telemetry" key={name}><Icon className="size-5 text-primary" /><span>{name}</span><b>{value}</b><small><span className="status-dot" />运行正常</small></div>)}</div>; }
...
function Weather() { const forecasts: Array<[string, string, LucideIcon]> = [["今天","28°",CloudSun],["周二","26°",CloudSun],["周三","24°",Droplets],["周四","27°",Wind]]; return <div className="grid grid-cols-4 gap-2 pt-4">{forecasts.map(([day,temp,Icon],i) => <div className={cn("weather-card", i===0 && "weather-active")} key={day}><span>{day}</span><Icon className="my-4 size-7" /><b>{temp}</b><small>{i===2 ? "小雨" : "多云"}</small></div>)}</div>; }
